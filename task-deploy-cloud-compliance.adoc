...
Si vous n'avez pas encore de connecteur, créez-en un maintenant. Voir https://["Création d'un connecteur dans AWS"^], https://["Création d'un connecteur dans Azure"^], ou https://["Création d'un connecteur dans GCP"^].

[role="quick-margin-para"]
Vous pouvez également https://["Installer le connecteur sur site"^] Sur un hôte Linux de votre réseau ou sur un hôte Linux du cloud.
...
Si vous n'avez pas encore de connecteur, créez-en un chez votre fournisseur cloud. Voir https://["Création d'un connecteur dans AWS"^] ou https://["Création d'un connecteur dans Azure"^], ou https://["Création d'un connecteur dans GCP"^]. Dans la plupart des cas, un connecteur sera probablement configuré avant d'essayer d'activer la classification BlueXP, car la plupart https://["Les fonctionnalités BlueXP nécessitent un connecteur"], mais il y a des cas où vous devrez en configurer un maintenant.
...
Notez que vous pouvez également https://["Installer le connecteur sur site"^] Sur un hôte Linux de votre réseau ou dans le cloud. Certains utilisateurs qui prévoient d'installer la classification BlueXP sur site peuvent également choisir d'installer le connecteur sur site.

Comme vous pouvez le voir, il peut y avoir des situations où vous devez utiliser https://["Plusieurs connecteurs"].
...
https://["Voir plus d'informations sur le déploiement du connecteur dans une région gouvernementale"^].
...
+
--
Des règles de groupes de sécurité supplémentaires sont nécessaires pour les déploiements AWS et AWS GovCloud. Voir https://["Règles pour le connecteur dans AWS"^] pour plus d'informations.

Des règles de groupes de sécurité entrantes et sortantes supplémentaires sont nécessaires pour les déploiements d'Azure et d'Azure Government. Voir https://["Règles pour le connecteur dans Azure"^] pour plus d'informations.
...
* https://["Documentation AWS : quotas de service Amazon EC2"^]
* https://["Documentation Azure : quotas de vCPU de machine virtuelle"^]
* https://["Documentation Google Cloud : quotas de ressources"^]
