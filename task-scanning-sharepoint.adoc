...
* Pour SharePoint Online, il est possible de détecter les données link:task-deploy-cloud-compliance.html["déploiement dans le cloud"^].
...
Lorsque Data SENSE est installé sur un site sans accès à Internet, le connecteur BlueXP doit également être installé sur ce même site sans accès à Internet. https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-quick-start-private-mode.html["En savoir plus >>"^].
