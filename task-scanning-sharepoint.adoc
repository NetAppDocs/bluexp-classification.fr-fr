...
Lorsque la classification BlueXP est installée sur un site sans accès Internet, le connecteur BlueXP doit également être installé sur ce même site sans accès Internet. https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-quick-start-private-mode.html["En savoir plus >>"^].
...
Vous pouvez ajouter des sites SharePoint individuels ou tous les sites SharePoint du compte, de sorte que les fichiers associés soient analysés par la classification BlueXP. Les étapes sont identiques lors de l'ajout de sites SharePoint Online ou SharePoint sur site.
