...
* **Cartographie uniquement** les scannages ne fournissent qu'une vue d'ensemble de haut niveau de vos données et sont effectués sur des sources de données sélectionnées. Les analyses de mappage uniquement prennent moins de temps que les analyses de mappage et de classification car elles n'accèdent pas aux fichiers pour voir les données qu'elles contiennent.
* **Les analyses Map and Classify** permettent une analyse approfondie de vos données.


Les analyses de mappage uniquement vous permettent d'analyser rapidement vos données et d'identifier les sources de données susceptibles de nécessiter davantage de recherche. Vous pouvez ensuite effectuer une analyse de mappage et de classification.
...
* https://docs.netapp.com/us-en/bluexp-setup-admin/reference-iam-predefined-roles.html["Rôles IAM BlueXP "] (Lors de l'utilisation de BlueXP  en mode standard)
* https://docs.netapp.com/us-en/bluexp-setup-admin/reference-user-roles.html["Rôles de compte BlueXP "^] (Lors de l'utilisation de BlueXP  en mode restreint ou privé)
