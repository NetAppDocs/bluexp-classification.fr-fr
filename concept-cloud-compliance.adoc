...
https://["Découvrez les utilisations de la classification BlueXP"^].
...
* Afficher et modifier https://["Étiquettes Azure information protection (AIP)"^] dans vos fichiers.
...
* Réduisez les coûts du stockage en identifiant les données inactives que vous pouvez déplacer vers un stockage objet moins coûteux. https://["En savoir plus sur le Tiering des systèmes Cloud Volumes ONTAP"^]. https://["En savoir plus sur le Tiering à partir des systèmes ONTAP sur site"^].
...
* https://["AWS : tarification Amazon EC2"^]
* https://["Microsoft Azure : détails de la tarification de la bande passante"^]
* https://["Google Cloud : tarification du service de transfert du stockage"^]
...
Lorsque vous déployez la classification BlueXP dans le cloud, BlueXP déploie l'instance dans le même sous-réseau que le connecteur. https://["En savoir plus sur les connecteurs."^]
...
* Dans AWS, la classification BlueXP s'exécute sur un https://["instance m6i.4xlarge"^] Avec un disque GP2 de 500 Gio. L'image du système d'exploitation est Amazon Linux 2. Lorsqu'elle est déployée dans AWS, vous pouvez choisir une instance de plus petite taille si vous analysez un petit volume de données.
...
| Grand (par défaut) | 16 processeurs, 64 Go de RAM, SSD de 500 Gio | Aucune 
...
|===
Lorsque vous déployez la classification BlueXP dans le cloud sur AWS, vous pouvez choisir une instance de grande/moyenne/petite. Lorsque vous déployez la classification BlueXP dans Azure ou GCP, envoyez un e-mail à ng-contact-data-sense@netapp.com pour obtenir de l'aide si vous souhaitez utiliser l'un de ces systèmes plus petits. Nous devons nous aider à déployer ces plus petites configurations cloud.

Lors du déploiement de la classification BlueXP sur site, il vous suffit d'utiliser un hôte Linux avec les spécifications les plus petites. Vous n'avez pas besoin de contacter NetApp pour obtenir de l'aide.
...
Une fois que vous avez activé la classification BlueXP et sélectionné les volumes, les compartiments, les schémas de base de données ou les données utilisateur OneDrive ou SharePoint que vous souhaitez analyser, l'analyse des données commence immédiatement pour identifier les données personnelles et sensibles. Il mappe les données de votre organisation, classe chaque fichier et identifie et extrait des entités et des modèles prédéfinis dans les données. Le résultat de l'analyse est un index des informations personnelles, des données personnelles sensibles, des catégories de données et des types de fichiers.
...
Après l'analyse initiale, la classification BlueXP analyse en continu vos données pour détecter les modifications incrémentielles (c'est pourquoi il est important de maintenir l'exécution de l'instance).
...
Catégories:: La classification BlueXP récupère les données qu'il a analysées et les divise en différents types de catégories. Les catégories sont des rubriques basées sur l'analyse par IA du contenu et des métadonnées de chaque fichier. link:task-controlling-private-data.html#viewing-files-by-categories["En savoir plus sur les catégories"^].
Types:: La classification BlueXP répartit les données analysées par type de fichier. link:task-controlling-private-data.html#viewing-files-by-file-types["En savoir plus sur les types"^].
...
https://["En savoir plus sur les rôles BlueXP"^] et comment https://["ajoutez des utilisateurs avec des rôles spécifiques"^].
