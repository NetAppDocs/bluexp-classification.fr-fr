...
https://bluexp.netapp.com/netapp-cloud-data-sense["Découvrez les utilisations de Cloud Data Sense"^].
...
* Dans AWS, Cloud Data Sense s'exécute sur un https://aws.amazon.com/ec2/instance-types/m6i/["instance m6i.4xlarge"^] Avec un disque GP2 de 500 Gio. L'image du système d'exploitation est Amazon Linux 2.
* Dans Azure, Cloud Data Sense s'exécute sur un link:https://docs.microsoft.com/en-us/azure/virtual-machines/dv3-dsv3-series#dsv3-series["Machine virtuelle standard_D16s_v3"^] Avec un disque de 500 Gio. L'image du système d'exploitation est CentOS 7.9.
* Dans GCP, Cloud Data Sense s'exécute sur un link:https://cloud.google.com/compute/docs/general-purpose-machines#n2_machines["n2-standard-16 VM"^] Avec un disque persistant standard de 500 Gio. L'image du système d'exploitation est CentOS 7.9.
* Dans les régions où l'instance par défaut n'est pas disponible, Data Sense s'exécute sur une autre instance. link:reference-instance-types.html["Voir les autres types d'instances"].
...
Vous pouvez également déployer Data Sense sur un hôte Linux sur site ou sur un hôte dans votre fournisseur de cloud préféré. Le logiciel fonctionne exactement de la même manière quelle que soit la méthode d'installation choisie. Les mises à niveau du logiciel Data Sense sont automatisées tant que l'instance a accès à Internet.
...
|===
Lorsque vous déployez Data Sense dans le cloud sur AWS, vous pouvez choisir une instance de grande, moyenne et petite taille. Lorsque vous déployez Data Sense dans Azure ou GCP, envoyez un e-mail à l'adresse ng-contact-data-sense@netapp.com pour obtenir de l'aide si vous souhaitez utiliser l'un de ces systèmes plus petits. Nous devons nous aider à déployer ces plus petites configurations cloud.
...
=== La rapidité avec laquelle les données de détection sont-elles à l'examen
...
* Lors de l'exécution d'analyses de mappage, Data SENSE peut analyser entre 100-150 tibites de données par jour et par nœud du scanner.
* Lors de l'exécution d'acquisitions de classification, Data Sense peut analyser entre 15-40 tibides de données par jour et par nœud d'analyse.


link:task-deploy-compliance-onprem.html#install-data-sense-on-the-linux-host["En savoir plus sur le déploiement de plusieurs nœuds de numérisation pour analyser vos données"^].
...
Reconnaissance de l'entité de nom:: Cloud Data Sense exploite l'IA pour extraire le nom des personnes physiques à partir des documents. link:task-generating-compliance-reports.html#what-is-a-data-subject-access-request["Découvrez comment répondre aux demandes d'accès aux données"^].
