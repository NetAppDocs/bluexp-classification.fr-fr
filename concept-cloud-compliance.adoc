...
Il est possible d'analyser entre 15-40 tibides de données par jour et par nœud du scanner. La vitesse de analyse est affectée par la latence du réseau, la latence des disques, la bande passante réseau, la taille de l'environnement et la taille de la distribution de fichiers.
...
Reconnaissance de l'entité de nom:: Cloud Data Sense exploite l'IA pour extraire le nom des personnes physiques à partir des documents. link:task-responding-to-dsar.html["Découvrez comment répondre aux demandes d'accès aux données"^].
