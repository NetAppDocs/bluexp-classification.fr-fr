...
| Chemin du fichier | Saisissez jusqu'à 20 chemins partiels ou complets que vous souhaitez inclure ou exclure de la requête. Si vous entrez à la fois inclure des chemins d'accès et exclure des chemins, Data Sense recherche d'abord tous les fichiers des chemins inclus, puis supprime les fichiers des chemins exclus, puis affiche les résultats. Notez que l'inclusion de "*" dans ce filtre n'a aucun effet ; vous devrez rechercher des fichiers et des sous-dossiers spécifiques en utilisant des chemins complets. 
...
| Événement d'analyse d'acquisition | Indiquez si vous souhaitez afficher les fichiers qui n'ont pas été classés car Data Sense n'a pas pu revenir à l'heure du dernier accès ou les fichiers classifiés même si Data Sense n'a pas pu rétablir l'heure du dernier accès. 
|===
