...
Si votre licence expire ou si vous avez atteint la limite BYOL, Data Sense continue de s'exécuter, mais l'accès aux tableaux de bord est bloqué afin que vous ne puissiez pas afficher les informations relatives à vos données numérisées. Seule la page _Configuration_ est disponible au cas où vous souhaitez réduire le nombre de volumes analysés afin d'augmenter votre capacité de stockage sous la limite de licence.
...
NOTE: Si le compte que vous utilisez possède à la fois une licence BYOL et un abonnement PAYGO, Data Sense _ne_ pas passer à l'abonnement PAYGO lorsque la licence BYOL expire. Vous devez renouveler la licence BYOL.
