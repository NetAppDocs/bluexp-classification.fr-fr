...
Si vous n'avez pas encore de connecteur, https://["Déployez le connecteur sur site"^] Sur un hôte Linux de votre réseau ou sur un hôte Linux dans le cloud.

[role="quick-margin-para"]
Vous pouvez également créer un connecteur avec votre fournisseur cloud. Voir https://["Création d'un connecteur dans AWS"^], https://["Création d'un connecteur dans Azure"^], ou https://["Création d'un connecteur dans GCP"^].
...
Un connecteur BlueXP est requis avant de pouvoir installer et utiliser la classification BlueXP. Dans la plupart des cas, vous aurez probablement configuré un connecteur avant d'essayer d'activer la classification BlueXP, car la plupart https://["Les fonctionnalités BlueXP nécessitent un connecteur"], mais il y a des cas où vous devrez en configurer un maintenant.

Pour en créer un dans votre environnement de fournisseur cloud, consultez la section https://["Création d'un connecteur dans AWS"^], https://["Création d'un connecteur dans Azure"^], ou https://["Création d'un connecteur dans GCP"^].
...
Notez que vous pouvez également https://["Déployez le connecteur sur site"^] Sur un hôte Linux de votre réseau ou sur un hôte Linux du cloud. Certains utilisateurs qui prévoient d'installer la classification BlueXP sur site peuvent également choisir d'installer le connecteur sur site.
...
.Étapes
. Téléchargez le logiciel de classification BlueXP depuis le https://["Site de support NetApp"^]. Le fichier que vous devez sélectionner est nommé *DATASESNSE-INSTALLER-<version>.tar.gz*.
...
. Accédez au https://["API Documentation Developer Hub"^] Et cliquez sur *Apprenez à vous authentifier*.
+
