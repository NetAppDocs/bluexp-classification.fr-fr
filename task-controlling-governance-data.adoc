...
La section _Data Overview_ fournit un aperçu rapide de toutes les données en cours d'acquisition. Cliquez sur le bouton pour télécharger un rapport de mappage de données complet incluant la capacité d'utilisation, l'âge des données, la taille des données et les types de fichiers pour tous vos environnements de travail et toutes vos sources de données. Voir link:task-generating-compliance-reports.html#data-mapping-report["Rapport de mappage de données"^] pour plus d'informations sur ce rapport.
