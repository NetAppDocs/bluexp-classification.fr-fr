...
.Avant de commencer
* Le logiciel Data Sense peut être mis à niveau une version majeure à la fois. Par exemple, si la version 1.17.x est installée, vous ne pouvez effectuer la mise à niveau que vers la version 1.18.x. Si vous êtes quelques versions principales derrière, vous devrez mettre à niveau le logiciel à plusieurs reprises.
